  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const Q1 = parsedData.mot[4];
        if (Math.abs(Q1) > Q1_THRESHOLD) {
          if (Q1 > 0) {
            console.log("Right");
      ...
      {questionObj && (
        <>
          ...
          <div className={styles["btn-group"]}>
            <button ref={leftButtonRef} onClick={() => handleAnswerClick(questionObj.answers[0])}>
              {questionObj.answers[0]}
            </button>
            <button ref={rightButtonRef} onClick={() => handleAnswerClick(questionObj.answers[1])}>
